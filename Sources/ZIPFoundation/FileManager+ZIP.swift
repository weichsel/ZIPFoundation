...
extension FileManager {
    ...
    public func unzipItem(at sourceURL: URL, to destinationURL: URL, skipCRC32: Bool = false,
                          progress: Progress? = nil, preferredEncoding: String.Encoding? = nil, replaceOnExist: Bool = false) throws {
        let fileManager = FileManager()
        guard fileManager.itemExists(at: sourceURL) else {
            throw CocoaError(.fileReadNoSuchFile, userInfo: [NSFilePathErrorKey: sourceURL.path])
        }
        ...
        for entry in sortedEntries {
            ...
            if let progress = progress {
                ...
                progress.addChild(entryProgress, withPendingUnitCount: entryProgress.totalUnitCount)
                _ = try archive.extract(entry, to: destinationEntryURL, skipCRC32: skipCRC32, progress: entryProgress, replaceOnExist: replaceOnExist)
            } else {
                _ = try archive.extract(entry, to: destinationEntryURL, skipCRC32: skipCRC32, replaceOnExist: replaceOnExist)
            }
        }
    }
